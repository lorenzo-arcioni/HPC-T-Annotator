...
then
	...
	if [ "$choice" = "1" ]; then
		...
		case "$wlm" in
			slurm)
			tar -cf hpc-t-annotator.tar read.py start.sh time_calculator.py cancel.sh checker.sh control_script.sh monitor.sh slurm_error_checker.sh
			;;
			htcondor)
			echo "Work in progress!!"
			...
			;;
			none)
			tar -cf hpc-t-annotator.tar read.py start.sh time_calculator.py control_script.sh
			;;
		esac 
